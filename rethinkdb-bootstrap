...
if [ $? != 0 ]; then
  ...
  export first="$(curl -L --fail $etcd_root/keys/rethinkdb.com/bootstrap/bootstrapped?quorum=true 2>/dev/null | jq '.node.value' | tr -d '\"')"
  echo "first =" $first

  others=$(curl -L --fail $etcd_root/keys/rethinkdb.com/bootstrap/machines/?quorum=true 2>/dev/null | jq '.node.nodes[].key' | while read line; do
